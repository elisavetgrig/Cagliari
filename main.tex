...
Thus ,% the aim of this paper is to develop an SD-based approach for QoE management of future multimedia services.In order to meet this goal, this paper develop a flexible, an efficient and scalable architecture with autonomous behavior in order to succeed high level of QoE in future networks.%Quality-of-Experience (QoE)// in future networks. 
%This architecture is based on the principles of network softwarization using techologies such as such as Software-Defined Networks (SDN) and Network Function Virtualization (NFV).
%
%we can remove this paragraph because the second unbundling using software/hardware decoupling and control/user plane separation, and (iii) Open using open source software and open interfaces.
...
%There is an Application \textit{app} in a set of \textit{K} applications. This application is described as a DAG of tasks: $G_T$ = (T, $E_T$), where T = $\{\ 1 \cdots ,\lambda, \cdots \Lambda \}\ $ is a set of the tasks and $E_T$ = ($e_{uw}$) is the set of edges. Each edge $e_{uw}$ representing a undirectional data transfer from task $_u$ to task $_w$. En example of the DAG is illustrated in Figure~\ref{fig:generictasks} %


%A binary vector s(i) = \lceil s(i, \lambda) \rfloor  for \lambda  \in Tasks 
...
The aim of optimization problem is to find the trade-off that best fits the requirements of both %should change that% 
